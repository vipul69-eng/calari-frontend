...
interface UserState {
  user: User | null
  userToken: string | null // Added to store token internally
  ...
  setUser: (user: User) => void // Keep original signature
  setUserToken: (token: string | null) => void // Helper method to set token separately
    ...
    immer((set, get) => ({
      ...
      setUser: (user) => {
        ...
        if (!currentUser || currentUser.id !== user.id) {
          set((state) => {
            ...
            state.userToken = null // Clear the stored token
          })
        }
      ...
      clearUser: () => {
        set((state) => {
          ...
          state.dailyNutrition = {} // Clear all data on logout
          state.currentDate = new Date().toISOString().split("T")[0]
          state.isSyncing = false
          state.lastSyncTime = null
        })
...
/* New hook to set token separately if needed */
